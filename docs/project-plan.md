...
这是一个面向 V 形导流区数据挖掘的多模态 AI 工作台：系统接收道路场景图像 / 图文样本后，复用既有微调模型作为前置过滤器，保持模型原始输出 `has_channelization + reason` 不变；外部增强框架再对 reason 进行解析，完成规则核验、风险判定和 keep / review / discard 筛选决策。低置信度、高风险或证据不足样本进入人工复核，同时支持批量评测、bad case 分析和结果回流。
...
3. V 形导流区筛选：复用既有模型输出 `has_channelization + reason`。
...
给定道路场景图像，判断是否存在轮廓完整可见、导流关系明确、整体为严格或近似 V 形，且不是 Y 形或多分叉结构的导流区。

### B. 严格负样本过滤

对普通车道线、路肩、单个设施、条带状缓冲区、轮廓不完整、遮挡模糊、Y 形或多分叉结构等负样本进行前置过滤。

### C. 图像 + 规则联合审查

给定图像、模型原始输出和规则模板，在不修改模型输出格式的基础上，派生复核信号、风险项和最终 keep / review / discard 决策。
...
- contour_complete
- v_shape_visible
- y_or_multi_fork_risk
- evidence_sufficient
- reason_quality_pass
- extracted_evidence
- risk_hints

关键要求：不改变既有微调模型输出格式，只在外部增加可解释、可核验、可复核的结构化信号。
...
- 必填字段缺失
- 模型判 true 但 reason 没有说明 V 形或楔形结构
- 模型判 true 但 reason 没有说明轮廓完整可见
- 模型判 true 但 reason 暗示 Y 形、三叉或多分叉
- 风险等级与解释不一致
- 结果不符合规则模板
...
上传样本 -> 微调模型输出 has_channelization + reason -> 外部框架派生复核信号 -> 规则核验 -> 风险判定 -> keep / review / discard 筛选决策 -> 高风险进入人工复核 -> 写回结果 -> 更新批量评测
